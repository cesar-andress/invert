...
def detect_eager_lazy(
    ...
    *,
    ...
    if cls is None:
        return _ambiguous(err or "load_failed")

    getter_a = _resolve_getter(getters, "feature_a")
    if getter_a is None:
        return _ambiguous("missing_getter")
    ...
    try:
        ...
        return _ambiguous(f"instantiation_failed: {exc}", trace=list(tracker.trace))
    ...
    getattr(pipeline, getter_a)()
    repeat_delta = _delta_counts(counts_pre_repeat, tracker.snapshot_counts())
    calls_after_repeat = sum(repeat_delta.values())
    ...
    if (
        calls_before == 0
        and calls_during == 1
        and set(computed_after_first) == {"feature_a"}
...
def detect_eager_lazy_file(
    ...
    *,
    ...
    return detect_eager_lazy(code, task=task, x=x)
