...
def recover(
    ...
    typer.echo(f"Recovery complete with judge: {judge_model}")
...
def aggregate(
    ...
    typer.echo(f"Aggregated results written to {results_dir}")
...
def plot(
    ...
    if run is not None:
        results_dir = PROJECT_ROOT / "results" / "runs" / run
    run_plot(results_dir)
    typer.echo(f"Heatmap written to {results_dir / 'identifiability_heatmap.png'}")
