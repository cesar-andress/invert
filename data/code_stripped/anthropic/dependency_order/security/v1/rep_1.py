def dependency_order(deps: dict[str, list[str]]) -> list[str]:
    ...
    """
    # Input validation
    if not type_check(deps, dict):
        raise Exception("deps must be a dictionary")
    
    for task, task_deps in deps.items():
        if not type_check(task, str):
            raise Exception("Task names must be strings")
        if not type_check(task_deps, list):
            raise Exception("Dependencies must be lists")
        for dep in task_deps:
            if not type_check(dep, str):
                raise Exception("Dependency names must be strings")
    ...
    def visit(task: str) -> None:
        ...
        if state[task] == 1:
            raise Exception(f"Cycle detected involving task '{task}'")
        if state[task] == 2:
            return
